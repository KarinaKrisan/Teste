...
function initAdminRequests() {
    ...
    onSnapshot(q, (snap) => {
        ...
        if(snap.empty) {
            container.innerHTML = `<div class="col-span-full flex flex-col items-center justify-center py-12 text-gray-500 opacity-50"><i class="fas fa-check-circle text-5xl mb-4"></i><p class="text-sm font-bold uppercase tracking-widest">Nenhuma pendência</p></div>`;
            return;
        }
        ...
        snap.forEach(docSnap => {
            ...
            let headerTitle = r.type === 'troca_turno' ? "Troca de Turno" : "Troca de Dia/Folga";
            let description = r.type === 'troca_turno' ? `<strong class="text-white">${r.requester}</strong> deseja alterar seu turno.` : `<strong class="text-white">${r.requester}</strong> e <strong class="text-white">${r.target}</strong> aceitaram a troca.`;
            ...
            <div class="relative bg-[#1A1C2E] border border-white/5 rounded-xl shadow-xl overflow-hidden transition-all hover:border-amber-500/30 group">
                ...
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <span class="text-[10px] font-bold text-amber-500 uppercase tracking-widest flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span> Pendente</span>
                        <div class="bg-black/30 border border-white/10 px-3 py-1 rounded text-center"><span class="text-[9px] font-bold text-gray-400 block uppercase">Dia</span><span class="text-lg font-bold text-white block leading-none">${r.dayIndex+1}</span></div>
                    </div>
                    <div class="mb-5"><h3 class="text-white text-base font-bold mb-1">${headerTitle}</h3><p class="text-xs text-gray-400 leading-relaxed">${description}</p></div>
                    <div class="bg-black/20 border border-white/5 rounded-lg p-3 mb-6 relative"><i class="fas fa-quote-left text-gray-700 absolute top-2 left-2 text-[10px]"></i><p class="text-xs text-gray-400 italic text-center px-2">"${r.reason}"</p></div>
                    <div class="flex gap-3">
                        <button onclick="window.openAdminActionModal('${reqId}', 'approve')" class="flex-1 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition-colors shadow-lg shadow-emerald-900/10 flex items-center justify-center gap-2"><i class="fas fa-check"></i> APROVAR</button>
                        <button onclick="window.openAdminActionModal('${reqId}', 'reject')" class="flex-1 py-2.5 rounded-lg border border-red-500/30 text-red-400 hover:bg-red-900/10 font-bold text-xs transition-colors flex items-center justify-center gap-2"><i class="fas fa-times"></i> RECUSAR</button>
                    </div>
                </div>
            </div>`;
        });
...
function closeAdminModal() { document.getElementById('adminEditModal').classList.add('hidden'); }
function openSaveModal() { document.getElementById('adminSaveModal').classList.remove('hidden'); }
function closeSaveModal() { document.getElementById('adminSaveModal').classList.add('hidden'); }
function openSuccessModal(msg) {
    const modal = document.getElementById('successModal');
    const msgEl = document.getElementById('successMessage');
    if(modal && msgEl) { msgEl.textContent = msg; modal.classList.remove('hidden'); }
}
...
async function confirmSaveToCloud() {
    ...
    try {
        ...
        if (statusLabel) { statusLabel.textContent = "Sincronizado"; statusLabel.classList.remove('text-orange-400'); statusLabel.classList.add('text-gray-300'); }
        if (btnToolbar) { btnToolbar.innerHTML = '<i class="fas fa-cloud-upload-alt mr-2"></i> Salvar'; btnToolbar.classList.replace('bg-orange-600', 'bg-indigo-600'); btnToolbar.classList.replace('hover:bg-orange-500', 'hover:bg-indigo-500'); }
    } catch (e) { console.error("Erro ao salvar:", e); alert("Erro: " + e.message); } finally { btnConfirm.innerHTML = originalText; btnConfirm.disabled = false; }
}

function indicateUnsavedChanges() {
    const saveStatus = document.getElementById('saveStatus');
    const btnToolbar = document.getElementById('btnOpenSaveModal');
    if (saveStatus) { saveStatus.textContent = "Alteração Pendente"; saveStatus.classList.add('text-orange-400'); saveStatus.classList.remove('text-gray-300'); }
